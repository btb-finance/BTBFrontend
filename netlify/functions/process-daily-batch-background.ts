...
const handler: Handler = async (event: ScheduledEvent, context: HandlerContext) => {
  console.log('Starting process-daily-batch-background function for BTB Finance (btb.finance)');
  console.log('Current time UTC:', new Date().toISOString());
  
  // Check if this is a status check request
  const isStatusCheck = event.queryStringParameters && event.queryStringParameters.status === 'true';
  
  // If this is a status check, return status information instead of starting background processing
  if (isStatusCheck) {
    try {
      ...
      const batchIndex = calculateBatchIndex();
      const isBatchProcessed = await contract.batchProcessed(batchIndex).catch(() => false);
      ...
      return {
        ...
        body: JSON.stringify({
          ...
          batchStatus: {
            batchIndex: batchIndex,
            isProcessed: isBatchProcessed,
            lastProcessed: new Date(processingTimeInfo.lastBatchTimestamp * 1000).toISOString(),
            processingInterval: `${processingTimeInfo.processingInterval} seconds (${processingTimeInfo.processingInterval / 3600} hours)`,
            nextValidTime: processingTimeInfo.nextValidTime.toISOString(),
            canProcessNow: processingTimeInfo.canProcessNow,
            secondsRemaining: processingTimeInfo.secondsUntilNextValid,
  ...
  // For non-status requests, return a 202 response immediately to indicate the function is running in the background
  const response = {
    statusCode: 202,
    body: JSON.stringify({
  ...
  if (event.httpMethod !== 'GET' && !process.env.IS_SCHEDULED) {
    console.log('Not a GET request or scheduled event. Skipping.');
    return response;
  }
  
  // For scheduled events, ensure we're in the processing window
  if (process.env.IS_SCHEDULED === "true" && !isWithinProcessingWindow()) {
  ...
  try {
    ...
    if ((event.httpMethod === 'GET' && !process.env.IS_SCHEDULED) || isGasTest) {
    ...
    // Log the result but don't wait for it to complete
    ...
    // Log errors but still return the 202 response since this is a background function
