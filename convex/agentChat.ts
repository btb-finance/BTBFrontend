...
const AUTO_FACTS = [
  ...
  "Safety: the auto wallet can only do what listed adapters allow. The agent can never withdraw, can never send tokens or positions anywhere but back to the owner, and every swap must be no worse than the pool's 10 minute average price less the slippage limit (1% on Base, 3% on Robinhood Chain). Before each rebalance the price must also be stable against that average. A pool that does not record price history cannot be rebalanced safely and is left alone. The agent is capped at a number of wallet actions per UTC day in each auto wallet: 150 for new wallets, 48 for early ones, up to 200, and a staked rebalance uses 3 (unstake, rebalance, restake). When the cap is reached the position waits until midnight UTC; the owner can raise it with 'Raise daily limit' on the auto position. Only the owner can withdraw, pause, change settings or upgrade, and only to versions BTB approved. Not audited yet.",
  "Buttons on an auto position: Increase liquidity (the user's tokens go into the auto wallet, which adds them to the same position, unstaking and restaking it if staked; auto-rebalance keeps running); Change interval; Pause; Stake or Unstake; Claim rewards; Auto-compound on or off; 'Withdraw leftover tokens' sends every spare token and any ETH in the auto wallet back to the user (spam tokens with no market price are left behind) while the position keeps running; 'Withdraw LP and stop auto' unstakes, sends the position and all leftovers back to the user's own wallet and stops auto-rebalance. Nothing is sold or closed.",
  ...
  "Topping up the app BTB balance: in the top-up panel (under the BTB balance), pay ETH or dollars on the chain the wallet is already on, with no bridge or swap: ETH or USDC on Base, ETH or USDG on Robinhood Chain, ETH, USDC, USDT or BTB on Ethereum, at least $1. It goes to the BTB Safe and is credited within seconds to the wallet that paid: BTB one for one, everything else at $0.00003 per BTB (for example $10 buys 333,333 BTB). The payment must come straight from that wallet. If a payment was not credited, paste its transaction hash in the same panel. The balance is only for app services (auto-rebalance, fast alerts, agent messages); it is not withdrawn as tokens.",
  "Why it may not rebalance: the price is right next to the range (a one-sided position waiting to be filled), the price is moving too fast, the cooldown after the last rebalance (10 minutes), the farm's 2 minute minimum stake time on Giga, the pool has no price history, the BTB balance is too low after the free actions are used, or the wallet's daily agent action limit is reached. The status line on each auto position says which.",
