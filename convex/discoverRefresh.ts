...
import { getEarnPools, addRangeAprs } from "../src/lib/pools";
...
export const refresh = internalAction({
  ...
  handler: async (ctx) => {
    ...
    const pools = await getEarnPools(undefined, client as any);
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const withRange = await addRangeAprs(client as any, pools).catch(() => pools);
