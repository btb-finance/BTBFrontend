...
async function tokenAddresses() {
  ...
  for (let page = 0; page < 12; page++) {
    const response = await fetch(url, { next: { revalidate: 45 }, signal: AbortSignal.timeout(12_000) });
...
export async function GET() {
  try {
    ...
    const settled = await Promise.allSettled(batches.map(async batch => {
      const response = await fetch(`${DEX}/${batch.join(',')}`, { next: { revalidate: 30 }, signal: AbortSignal.timeout(12_000) });
    ...
    const markets = [...best.values()].map(pair => ({
