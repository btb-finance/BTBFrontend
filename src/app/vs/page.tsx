          ...
          <tbody>
            {COMPETITORS.map((c) => (
              <tr key={c.slug}>
                ...
                <td style={styles.td}><Link href={`/vs/${c.slug}`} style={{ color: '#1DE9B6', fontWeight: 700, textDecoration: 'none' }}>Compare</Link></td>
              </tr>
            ))}
            <tr><td style={styles.tdStrong}>BTB Finance</td><td style={styles.tdStrong}>$0, plus a revenue share every Friday</td><td style={styles.td}><Link href="/simulate" style={{ color: '#1DE9B6', fontWeight: 700, textDecoration: 'none' }}>Open</Link></td></tr>
          </tbody>
        </table>
      </div>
