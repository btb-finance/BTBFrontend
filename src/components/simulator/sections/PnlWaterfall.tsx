    ...
    >
      <div style={{ border: btb.borderSoft, borderRadius: 16, overflow: 'hidden' }}>
        {legs.map((leg, i) => {
          ...
          return (
            <div key={leg.label} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: isFinal ? '14px 14px' : '11px 14px', background: isFinal ? 'rgba(82,227,164,0.08)' : i % 2 ? 'rgba(var(--fg-rgb), 0.025)' : 'transparent', borderTop: i ? '1px solid rgba(var(--fg-rgb), 0.07)' : 'none' }}>
