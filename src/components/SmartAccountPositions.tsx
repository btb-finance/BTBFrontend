  ...
  return <div style={{ display: 'flex', flexDirection: 'column', gap: 9 }}>
    ...
    {positions.map(item => {
      ...
      return <Glass key={p.id.toString()} padding={isMobile ? 12 : 16} radius={18}>
        ...
        {policy && <div style={{ marginTop: 8, padding: '9px 11px', borderRadius: 12, display: 'flex', flexWrap: 'wrap', gap: '6px 12px', background: active ? 'rgba(82,227,164,.035)' : 'rgba(255,255,255,.025)', border: btb.borderSoft }}><span style={{ color: active ? btb.green : btb.textDim, fontSize: 10.5, fontWeight: 800 }}>{active ? '● Guarded automation' : '○ Automation stopped'}</span><span style={{ color: btb.textMuted, fontSize: 10.5 }}>Slippage ≤ {policy.maximumSlippageBps / 100}%</span><span style={{ color: btb.textMuted, fontSize: 10.5 }}>Target {policy.targetTickWidth.toLocaleString()} ticks</span><span style={{ color: btb.textMuted, fontSize: 10.5 }}>Allowed {policy.minimumTick.toLocaleString()} to {policy.maximumTick.toLocaleString()}</span></div>}
