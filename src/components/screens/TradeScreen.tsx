...
import { useSmartTrade } from '../../lib/SmartTradeContext';
import { SmartTradePanel } from '../SmartTradePanel';
import { isAddress } from 'viem';
import { type Tab } from '../types';
import { useSidebar } from '../../lib/SidebarContext';
import type { MarketFeedData } from '../../lib/marketFeed';
...
export function TradeScreen({ address, onConnectWallet, marketFeed }: {
  ...
  const { selectTrade, tradeButtonState, presets, setTradeStatus } = useSmartTrade();
        ...
        {loading ? <div style={{ minHeight: 180, display: 'grid', placeItems: 'center' }}><Spinner size={22}/></div>
          ...
          : filtered.slice(0, visible).map((market, index) => isMobile ? (
            ...
            <div key={market.address} style={{ display: 'grid', gridTemplateColumns: 'minmax(190px,1.65fr) .75fr .58fr .72fr .72fr .58fr 126px', gap: 10, alignItems: 'center', minHeight: 62, padding: '0 12px', borderBottom: index < Math.min(visible, filtered.length) - 1 ? btb.borderSoft : undefined }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 5 }}>{(['buy', 'sell'] as const).map(side => { const s = tradeButtonState(market.address, side); return <button key={side} onClick={() => selectTrade(market, side)} disabled={s.disabled} style={{ ...marketButton(side), opacity: s.disabled ? .65 : 1, cursor: s.disabled ? 'default' : 'pointer' }}>{s.label}</button>; })}</div>
            </div>
          ))}
      </div>
