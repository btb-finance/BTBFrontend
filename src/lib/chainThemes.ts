...
export function chainThemeCss(theme: ChainTheme, mode: ColorMode = 'dark'): Record<string, string> {
  ...
  if (mode === 'light') {
    return {
      ...
      '--chain-app-background': 'radial-gradient(900px 600px at 8% -10%, rgba(139,92,246,.16), transparent 60%), radial-gradient(800px 600px at 100% 0%, rgba(56,189,248,.14), transparent 60%), #F6F7FA',
    };
  }
  return {
    '--color-mode': 'dark',
    ...
    '--chain-app-background': 'radial-gradient(900px 600px at 8% -10%, rgba(139,92,246,.22), transparent 60%), radial-gradient(800px 600px at 100% 0%, rgba(56,189,248,.16), transparent 60%), radial-gradient(700px 500px at 50% 110%, rgba(124,140,255,.12), transparent 60%), #0A0A0F',
  };
}
